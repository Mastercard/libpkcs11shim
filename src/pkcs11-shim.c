...
{
	fprintf(shim_output, "[in] %s ", name);
	print_generic(shim_output, 0, data, size, NULL);
}

static void
shim_dump_string_out(const char *name, CK_VOID_PTR data, CK_ULONG size)
{
	fprintf(shim_output, "[out] %s ", name);
	print_generic(shim_output, 0, data, size, NULL);
}
...
shim_attribute_req_in(const char *name, CK_ATTRIBUTE_PTR pTemplate,
			  CK_ULONG  ulCount)
{
	fprintf(shim_output, "[in] %s[%ld]: \n", name, ulCount);
	print_attribute_list_req(shim_output, pTemplate, ulCount);
}
...
{
	fprintf(shim_output, "[in] %s[%ld]: \n", name, ulCount);
	print_attribute_list(shim_output, pTemplate, ulCount);
}
...
shim_C_Initialize(CK_VOID_PTR pInitArgs)
{
	...
	if (pInitArgs) {
		...
		fprintf(shim_output, "     flags: %ld\n", ptr->flags);
		if (ptr->flags & CKF_LIBRARY_CANT_CREATE_OS_THREADS)
			fprintf(shim_output, "       CKF_LIBRARY_CANT_CREATE_OS_THREADS\n");
		if (ptr->flags & CKF_OS_LOCKING_OK)
			fprintf(shim_output, "       CKF_OS_LOCKING_OK\n");
	}
...
shim_C_GetInfo(CK_INFO_PTR pInfo)
{
	...
	if(rv == CKR_OK) {
		...
		print_ck_info(shim_output, pInfo);
	}
	return retne(rv);
}
		...
		CK_ULONG_PTR pulCount)
{
	...
	if(rv == CKR_OK) {
		...
		print_slot_list(shim_output, pSlotList, *pulCount);
...
shim_C_GetSlotInfo(CK_SLOT_ID slotID, CK_SLOT_INFO_PTR pInfo)
{
	...
	if(rv == CKR_OK) {
		shim_dump_desc_out("pInfo");
		print_slot_info(shim_output, pInfo);
	}
	return retne(rv);
}
			 ...
			 CK_TOKEN_INFO_PTR pInfo)
{
	...
	if(rv == CKR_OK) {
		shim_dump_desc_out("pInfo");
		print_token_info(shim_output, pInfo);
	}
	return retne(rv);
}
		...
		CK_ULONG_PTR  pulCount)
{
	...
	if(rv == CKR_OK) {
		...
		print_mech_list(shim_output, pMechanismList, *pulCount);
	}
	return retne(rv);
}
		...
		CK_MECHANISM_INFO_PTR pInfo)
{
	...
	shim_dump_ulong_in("slotID", slotID);
	if (name)
		fprintf(shim_output, "%30s \n", name);
	else
		fprintf(shim_output, " Unknown Mechanism (%08lx)  \n", type);

	rv = po->C_GetMechanismInfo(slotID, type, pInfo);
	if(rv == CKR_OK) {
		shim_dump_desc_out("pInfo");
		print_mech_info(shim_output, type, pInfo);
	}
	return retne(rv);
}
		...
		CK_NOTIFY  Notify, CK_SESSION_HANDLE_PTR phSession)
{
	...
	fprintf(shim_output, "pApplication=%p\n", pApplication);
	fprintf(shim_output, "Notify=%p\n", (void *)Notify);
...
shim_C_GetSessionInfo(CK_SESSION_HANDLE hSession, CK_SESSION_INFO_PTR pInfo)
{
	...
	if(rv == CKR_OK) {
		shim_dump_desc_out("pInfo");
		print_session_info(shim_output, pInfo);
	}
	return retne(rv);
}
		...
		CK_UTF8CHAR_PTR pPin, CK_ULONG  ulPinLen)
{
	...
	enter("C_Login");
	shim_dump_ulong_in("hSession", hSession);
	fprintf(shim_output, "[in] userType = %s\n",
		...
		CK_ULONG_PTR  pulObjectCount)
{
	...
	if (rv == CKR_OK) {
		...
		for (i = 0; i < *pulObjectCount; i++)
			fprintf(shim_output, "Object 0x%lx matches\n", phObject[i]);
	}
	return retne(rv);
}
...
shim_C_EncryptInit(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
	...
	case CKM_AES_GCM:
		if (pMechanism->pParameter != NULL) {
			...
			fprintf(shim_output, "pMechanism->pParameter->ulTagBits=%lu\n", param->ulTagBits);
		} else {
			fprintf(shim_output, "Parameters block for %s is empty...\n",
				lookup_enum(MEC_T, pMechanism->mechanism));
		}
		break;
	default:
...
shim_C_DecryptInit(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
	...
	case CKM_RSA_PKCS_OAEP:
		if (pMechanism->pParameter != NULL) {
			...
			fprintf(shim_output, "pMechanism->pParameter->hashAlg=%s\n",
				lookup_enum(MEC_T, param->hashAlg));
			fprintf(shim_output, "pMechanism->pParameter->mgf=%s\n",
				lookup_enum(MGF_T, param->mgf));
			fprintf(shim_output, "pMechanism->pParameter->source=%lu\n", param->source);
...
shim_C_SignInit(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
	...
	case CKM_SHA512_RSA_PKCS_PSS:
		if (pMechanism->pParameter != NULL) {
			...
			fprintf(shim_output, "pMechanism->pParameter->mgf=%s\n",
				lookup_enum(MGF_T, param->mgf));
			fprintf(shim_output, "pMechanism->pParameter->sLen=%lu\n",
				param->sLen);
		} else {
			fprintf(shim_output, "Parameters block for %s is empty...\n",
				lookup_enum(MEC_T, pMechanism->mechanism));
		...
		CK_ATTRIBUTE_PTR pTemplate, CK_ULONG  ulAttributeCount, CK_OBJECT_HANDLE_PTR phKey)
{
	...
	enter("C_DeriveKey");
	shim_dump_ulong_in("hSession", hSession);
	fprintf(shim_output, "[in] pMechanism->type=%s\n",
		lookup_enum(MEC_T, pMechanism->mechanism));
	switch (pMechanism->mechanism) {
	case CKM_ECDH1_DERIVE:
	case CKM_ECDH1_COFACTOR_DERIVE:
		if (pMechanism->pParameter == NULL) {
			fprintf(shim_output, "[in] pMechanism->pParameter = NULL\n");
			break;
		}
		CK_ECDH1_DERIVE_PARAMS *param =
			(CK_ECDH1_DERIVE_PARAMS *) pMechanism->pParameter;
		fprintf(shim_output, "[in] pMechanism->pParameter = {\n\tkdf=%s\n",
			lookup_enum(CKD_T, param->kdf));
		fprintf(shim_output, "\tpSharedData[ulSharedDataLen] = ");
		print_generic(shim_output, 0, param->pSharedData,
			param->ulSharedDataLen, NULL);
		fprintf(shim_output, "\tpPublicData[ulPublicDataLen] = ");
		print_generic(shim_output, 0, param->pPublicData,
	...
	case CKM_ECMQV_DERIVE:
		...
		fprintf(shim_output, "\tpSharedData[ulSharedDataLen] =");
		print_generic(shim_output, 0, param2->pSharedData,
			param2->ulSharedDataLen, NULL);
		fprintf(shim_output, "\tpPublicData[ulPublicDataLen] = ");
		print_generic(shim_output, 0, param2->pPublicData,
			param2->ulPublicDataLen, NULL);
		fprintf(shim_output, "\tulPrivateDataLen = %lu",
			param2->ulPrivateDataLen);
		fprintf(shim_output, "\thPrivateData = %lu", param2->hPrivateData);
		fprintf(shim_output, "\tpPublicData2[ulPublicDataLen2] = ");
		print_generic(shim_output, 0, param2->pPublicData2,
			param2->ulPublicDataLen2, NULL);
		fprintf(shim_output, "\tpublicKey = %lu", param2->publicKey);
		fprintf(shim_output, "}\n");
		break;
	}
...
init_shim(void)
{
	...
	pkcs11_shim = malloc(sizeof(CK_FUNCTION_LIST));
	if (pkcs11_shim) {
	 ...
	 */
	output = getenv("PKCS11SHIM_OUTPUT");
	if (output)
		shim_output = fopen(output, "a");
	...
	if (!shim_output) {
		 ...
		 */
		rc = RegOpenKeyEx( HKEY_LOCAL_MACHINE, "Software\\OpenSC Project\\PKCS11-Shim", 0, KEY_QUERY_VALUE, &hKey );
		if (rc != ERROR_SUCCESS )
			rc = RegOpenKeyEx( HKEY_CURRENT_USER, "Software\\OpenSC Project\\PKCS11-Shim", 0, KEY_QUERY_VALUE, &hKey );
	...
	module = getenv("PKCS11SHIM");
#ifdef _WIN32
	if (!module) {
		/* try for the machine version first, as we may be running
		 * without a user during login
		 */
		rc = RegOpenKeyEx( HKEY_LOCAL_MACHINE, "Software\\OpenSC Project\\PKCS11-Shim",
				0, KEY_QUERY_VALUE, &hKey );
		if (rc != ERROR_SUCCESS)
			rc = RegOpenKeyEx( HKEY_CURRENT_USER, "Software\\OpenSC Project\\PKCS11-Shim",
					0, KEY_QUERY_VALUE, &hKey );
...
#endif
	if (module == NULL) {
		fprintf(shim_output, "Error: no module specified. Please set PKCS11SHIM environment.\n");
		free(pkcs11_shim);
