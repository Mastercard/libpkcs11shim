...
void print_enum(FILE *f, CK_LONG type, CK_VOID_PTR value, CK_ULONG size, CK_VOID_PTR arg)
{
    ...
    deferred_fprintf(f, "Value %lX not found for type %s\n", ctype, spec->name);
}
...
static void _print_generic(FILE *f, CK_LONG type, CK_VOID_PTR value, CK_ULONG size, CK_VOID_PTR arg, bool reveal)
{
	...
	if (value != NULL)
	    ...
	    deferred_fprintf(f, "NULL [size : 0x%lX (%ld)]", size, size);
    }
    deferred_fprintf(f, "\n");
}
...
type_spec ck_attribute_specs[] = {
    ...
    { CKA_TRUSTED           , "CKA_TRUSTED          ", print_boolean, NULL },
    ...
    { CKA_SECONDARY_AUTH    , "CKA_SECONDARY_AUTH   ", print_boolean, NULL },
    ...
    { CKA_WRAP_WITH_TRUSTED , "CKA_WRAP_WITH_TRUSTED ", print_boolean, NULL },
    ...
    { CKA_RESET_ON_INIT     , "CKA_RESET_ON_INIT    ", print_boolean, NULL },
    { CKA_HAS_RESET         , "CKA_HAS_RESET        ", print_boolean, NULL },
...
print_ck_info(FILE *f, CK_INFO *info)
{
    ...
    deferred_fprintf(f, "      flags:                   %0lx\n",     info->flags );
    deferred_fprintf(f, "      libraryDescription:     '%32.32s'\n",  info->libraryDescription );
    deferred_fprintf(f, "      libraryVersion:          %d.%d\n",    info->libraryVersion.major, info->libraryVersion.minor );
}
...
print_slot_info(FILE *f, CK_SLOT_INFO *info)
{
    ...
    deferred_fprintf(f, "      slotDescription:        '%32.32s'\n",  info->slotDescription );
    deferred_fprintf(f, "                              '%32.32s'\n",  info->slotDescription+32 );
    deferred_fprintf(f, "      manufacturerID:         '%32.32s'\n",  info->manufacturerID );
    deferred_fprintf(f, "      hardwareVersion:         %d.%d\n",    info->hardwareVersion.major, info->hardwareVersion.minor );
    deferred_fprintf(f, "      firmwareVersion:         %d.%d\n",    info->firmwareVersion.major, info->firmwareVersion.minor );
    deferred_fprintf(f, "      flags:                   %0lx\n",     info->flags );
...
print_token_info(FILE *f, CK_TOKEN_INFO *info)
{
    ...
    deferred_fprintf(f, "      label:                  '%32.32s'\n",  info->label );
    deferred_fprintf(f, "      manufacturerID:         '%32.32s'\n",  info->manufacturerID );
    deferred_fprintf(f, "      model:                  '%16.16s'\n",  info->model );
    deferred_fprintf(f, "      serialNumber:           '%16.16s'\n",  info->serialNumber );
    deferred_fprintf(f, "      ulMaxSessionCount:       %ld\n",       info->ulMaxSessionCount );
    deferred_fprintf(f, "      ulSessionCount:          %ld\n",       info->ulSessionCount );
    deferred_fprintf(f, "      ulMaxRwSessionCount:     %ld\n",       info->ulMaxRwSessionCount );
    deferred_fprintf(f, "      ulRwSessionCount:        %ld\n",       info->ulRwSessionCount );
    deferred_fprintf(f, "      ulMaxPinLen:             %ld\n",       info->ulMaxPinLen );
    deferred_fprintf(f, "      ulMinPinLen:             %ld\n",       info->ulMinPinLen );
    deferred_fprintf(f, "      ulTotalPublicMemory:     %ld\n",       info->ulTotalPublicMemory );
    deferred_fprintf(f, "      ulFreePublicMemory:      %ld\n",       info->ulFreePublicMemory );
    deferred_fprintf(f, "      ulTotalPrivateMemory:    %ld\n",       info->ulTotalPrivateMemory );
    deferred_fprintf(f, "      ulFreePrivateMemory:     %ld\n",       info->ulFreePrivateMemory );
    deferred_fprintf(f, "      hardwareVersion:         %d.%d\n",     info->hardwareVersion.major, info->hardwareVersion.minor );
    deferred_fprintf(f, "      firmwareVersion:         %d.%d\n",     info->firmwareVersion.major, info->firmwareVersion.minor );
    deferred_fprintf(f, "      time:                   '%16.16s'\n",  info->utcTime );
    deferred_fprintf(f, "      flags:                   %0lx\n",      info->flags );
...
print_session_info(FILE *f, CK_SESSION_INFO *info)
{
    ...
    deferred_fprintf(f, "      slotID:                  %ld\n",       info->slotID );
    deferred_fprintf(f, "      state:                  '%32.32s'\n",  lookup_enum(STA_T, info->state));
    deferred_fprintf(f, "      flags:                   %0lx\n",     info->flags );
